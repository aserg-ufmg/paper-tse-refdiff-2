...
For all this reasons, in this paper we propose a multi-language refactoring detection approach, named as RefDiff~2.0, which is a complete redesign of its first version that introduces an extensible architecture.
...
As a way to validate this architecture, we implemented and evaluated extension modules for three mainstream programming languages: Java, C, and JavaScript.
Additionally, we reworked the refactoring detection heuristics of RefDiff to significantly improve its precision when compared to its prior version.
