...
\begin{figure}[htb]
\begin{algorithmic}[1]
\Procedure{FindRelationships}{$v_1,v_2$}
...
Moreover, there is a \textit{Extract} relationship indicating that the method \codeinl{print} was extract from \codeinl{main}.
...
%Specifically, the conditions from Table~\ref{TabRelationshipTypes} that contains the expression $\rdparent(n_1)'$, which denotes the matching element of the parent of $n_1$, relies on the matching relationships found so far.
